...
\section{Определенный интеграл}
    ...
    \subsection{Определённый интеграл}
        \subsubsection{Положительная и отрицательная срезки}
...
\subsection{Следствие о точках разрыва} 
...
\section{Продолжение опр интеграла}
    ...
    $Segm<a,b> = \lbrace[x,y]:x,y \in <a,b>,x \leq y \rbrace$ \\
    
     \subsection{Определение}
     1) $f: Segm<a,b> \rightarrow \overline{{\mathbb{R}}$-функция промежутка \\
     2) Аддитивная фп \\
      $\forall [p,q] \in Sign<a,b> \forall c \in(p,q) \\
     f([p,q])= f([p,c])+f([c,q])$\\
     3) Плотность аддитивной функции промежутка \\
     f: Segm<>a,b $\rightarrow \mathbb{R}$\\
     g-плотнотсь $\forall \vartriangle \in Segm <a,b> (inf (g))l(\vartriangle)\leq F\leq(sup (g) )l(\vartriangle) ,l(\vartriangle)$- длина $\vartriangle)$\\
...
\subsection{Площадь криволинейного сектора в  полярных координтах}
$r > 0, \phi \in [0,1]$ \\
...
\subsection{Кардиоида}
    ...
    $S = \frac{1}{2}\int_{0}^{2\pi}(2cos(\phi)-2cos( 2\phi )( 2cos( \phi )-cos( 2\phi )) + (2sin\phi -2 sin(2\phi))(2sin\phi - sin(2\phi)d\phi) = \frac{r^2}{2}\int_{0}^{2\pi}4 -6( cos(2\phi)cos(\phi) + sin(2\phi)sin(\phi)) +2 d\phi = \frac{r^2}{2}6\pi * 2 - 3r^2\int_{0}^{2\pi}cos\phi d\phi = \[ \boxed{\boxed{6\pi r^2 $}}\\
     \]
    \\
\textbf{Изопериметрическое неравенство}\\
      $G \in \mathbb{R}^2$ -замкнутая выпуклая фигура
     $diam G\leq 1$\\ Тогда $\Segma(G) \leq \frac{\pi}{4}$\\
 \textbf{Докзаательство}\\
    $\forall \phi , sup(r : (r, \phi) \in G )= f(\phi)$ \\
    $\Segma(G)=\int_{-\frac{\pi}{2}}^{\frac{\pi}{2}}f^2(\phi) d\phi = \frac{1}{2}\int_{0}^{\frac{\pi}{2}}f^2(\phi)+\int_{-\frac{\pi}{2}}^{0}f^2(\phi)=\frac{1}{2}\int_{0}^{\frac{\pi}{2}}f^2(\phi) + f^2(\phi-\frac{\pi}{2})   d\phi$ \\
\\
...
\texbf{Определние.} Функция l определена множестве главных путей $\mathbb{R}^m$ называется длиной, если: 
 ...
 \texbf{Пример.Эллипс}\\
    ...
    \texbf{ПримПлотность аддитивной функции промежуткаер.В полярных координатах} \nl
    $x=rcos\phi$  $y=rsin\phi$\\
    Параметризуем (дописать)
 \subsection{Лемма об общем виде функции ограниченной вариации}
    $Var_{a}^{b} f < +\infty$\\
...
$ \sum_{i=1}^n f(\xi_i)(x_i -x_{i -1})$- интегральная сумма(= Риманова сумма ) \nl
...
Другие - равномерные $ \frac{b -a }{n} $ - для каждой части \nl
  ...
  \subsection{еорема об интегральных суммах для центральных прямоугольников}
...
\subsection{Теорема о формуле трапеций}
